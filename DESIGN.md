...
- Site mark (`public/favicon.svg`, `.ico`, `apple-touch-icon.png`) — the nameplate's own Archivo
  600 `R` trailed by its check bit in verified gold: the ECC word's "byte, then parity" read as a
  monogram. No strike — at 16px a red slash reads as "error", not as a caught bit. Generated by
  `scripts/build-icons.mjs` so the letterform stays identical to the nameplate; one mark on every
  page and every surface.
