---
organization: TRUE (Trustworthy and Responsible) AI Lab
parent: Oregon State University
role: Undergraduate Researcher, URSA Engage — LLM Reliability & Safety
location: Corvallis, OR
start: 2025-11-01
stack: [PyTorch, Hugging Face, Slurm]
summary:
  problem: "Hardware bit-flips can silently corrupt LLM inference — how robust are models, and what defends them?"
  built: "Robustness evaluations on the lab’s fault-injection harness, run at scale on an A40 Slurm cluster."
  result: "Benchmarked 7 defenses against 2 attack classes across GSM8K, DROP and TriviaQA; co-authored a cross-scale resilience manuscript, now under review."
order: 2
---

- Evaluate robustness of Llama-3-8B to hardware bit-flip faults using the lab's fault-injection harness (PyTorch, Hugging Face), run at scale on an A40 Slurm HPC cluster
- Benchmarked 7 fault-tolerance defense methods against 2 classes of adversarial bit-flip attacks (stealth and damage) across GSM8K, DROP and TriviaQA
- Co-authored manuscript on cross-scale model resilience across 8B–32B target models, now under review; contributed experiments, benchmarks and the accuracy-versus-fault-tolerance trade-off analysis
